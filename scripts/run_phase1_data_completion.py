...
def step_gate(data_dir: Path, repo_dir: Path, pv_images_root: Path,
              threshold: int = PHASH_THRESHOLD) -> dict:
    # Exact-exclusion credit is derived from records, never from a caller
    # integer (AUD-EC-002), so there is no excluded_pairs parameter.
    from ica26.leakage.gate import compute_gate, write_run_report
    ...
    write_run_report(gate, repo_dir / "reports" / "leakage_gate_run.json",
                     command="run_phase1_data_completion.py:gate")
    log(f"gate: status={gate.status} "
